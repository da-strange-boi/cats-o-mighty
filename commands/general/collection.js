...
module.exports.run = async (bot, message, args) => {
    ...
    Cat.findOne({
        ...
        uBandit = catList.bandit;uBug = catList.bug;uLinda = catList.linda;uMittens = catList.mittens;uCash = catList.cash;uJackson = catList.jackson;uCottonball = catList.cottonball;uSonny = catList.sonny;uSmokey = catList.smokey;uLailah = catList.lailah;uCher = catList.cher;uMarvin = catList.marvin;
        ...
        if(uBandit === 0 && uBug === 0 && uLinda === 0 && uMittens === 0 && uCash === 0 && uJackson === 0 && uCottonball === 0 && uSonny === 0 && uSmokey === 0 && uLailah === 0 && uCher === 0 && uMarvin === 0){ specialCats = false;} else { specialCats = true;}
        ...
        if(args[0]) {
            ...
            if(catType === "common"){
                if(commonCats === false){message.channel.send(`<@${message.author.id}> you don't have any common cats to look at! sad uwu`); return; }
                //* Make Embed To Display The Cats The User Has
                let catsEmbed = new Discord.RichEmbed()
                .setAuthor(message.author.username + "'s common cat collection!")
                .setColor(config.color.cats)
                .addField(":heart: Common :heart:", `Siamese: ${uSiamese}\nBurmese: ${uBurmese}\nRagdoll: ${uRagdoll}\nPersian: ${uPersian}\nMaine Coon: ${uMaineCoon}\nRussian Blue: ${uRussianBlue}`, true);
                message.channel.send(catsEmbed);
                return;
            }
            if(catType === "uncommon"){
                if(uncommonCats === false){ message.channel.send(`<@${message.author.id}> you don't have any uncommon cats to look at! sad uwu`); return; }
                //* Make Embed To Display The Cats The User Has
                let catsEmbed = new Discord.RichEmbed()
                .setAuthor(message.author.username + "'s uncommon cat collection!")
                .setColor(config.color.cats)
                .addField(":blue_heart: Uncommon :blue_heart:", `Abyssiniam: ${uAbyssinian}\nManx: ${uManx}\nSphynx: ${uSphynx}\nCyprus: ${uCyprus}\nFoldex: ${uFoldex}`, true);
                message.channel.send(catsEmbed);
                return;
            }
            if(catType === "rare"){
                if(rareCats === false){ message.channel.send(`<@${message.author.id}> you don't have any rare cats to look at! sad uwu`); return; }
                //* Make Embed To Display The Cats The User Has
                let catsEmbed = new Discord.RichEmbed()
                .setAuthor(message.author.username + "'s rare cat collection!")
                .setColor(config.color.cats)
                .addField(":yellow_heart: Rare :yellow_heart:", `Korat: ${uKorat}\nSingapura: ${uSingapura}\nTonkinese: ${uTonkinese}\nPeterbald: ${uPeterbald}\nChartreux: ${uChartreux}\nMunchkin: ${uMunchkin}`, true);
                message.channel.send(catsEmbed);
                return;
            }
            if(catType === "special"){
                if(specialCats === false){ message.channel.send(`<@${message.author.id}> you don't have any special cats to look at! sad uwu`); return; }
                //* Make Embed To Display The Cats The User Has
                let catsEmbed = new Discord.RichEmbed()
                .setAuthor(message.author.username + "'s special cat collection!")
                .setColor(config.color.cats)
                .addField(":sparkling_heart: Special :sparkling_heart:", `Smokey: ${uSmokey}\nBandit: ${uBandit}\nBug: ${uBug}\nLinda: ${uLinda}\nMittens: ${uMittens}\nCash: ${uCash}\nJackson: ${uJackson}\nCottonball: ${uCottonball}\nSonny: ${uSonny}\nLailah: ${uLailah}\nCher: ${uCher}\nMarvin: ${uMarvin}`, true);
                message.channel.send(catsEmbed);
                return;
            }
            if(catType === "impossible"){
                if(impossibleCats === false){message.channel.send(`<@${message.author.id}> you don't have any impossible cats to look at! sad uwu`); return; }
                //* Make Embed To Display The Cats The User Has
                let catsEmbed = new Discord.RichEmbed()
                .setAuthor(message.author.username + "'s impossible cat collection!")
                .setColor(config.color.cats)
                .addField(":gem: Impossible :gem:", `Squirtlett: ${uSquirtlett}\nCursed Cat: ${uCursedcat}`, true);
                message.channel.send(catsEmbed);
                return;
            }
        }
