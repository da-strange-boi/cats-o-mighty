...
module.exports.run = async (bot, message, args) => {
  ...
  if(args[0]){
    ...
    const displayEmbed = async (commandName, args, whatitdoes, example, aliases) => {
      const embed = new Discord.RichEmbed()
      .setColor(config.color.utility)
      .setTitle(`${commandName} Help Page`)
      .addField('Usage', `cat ${commandName} ${args}`)
      .addField('What it does', `${whatitdoes}`)
    ...
    if(helpCommand === "collection"){
      displayEmbed('collection', '[rank of cat]', 'shows you your cat collection', 'cat collection common', '**cattos**, **c**');
    }
